...
def index(request):
    ...
    start_date_no = datetime.datetime.now().strftime("%Y%m%d")
    end_date_date = datetime.datetime.now() + datetime.timedelta(days=crawl_days - 1)
    end_date = (
        (end_date_date).strftime("%Y{y}%m{m}%d{d}").format(y="年", m="月", d="日")
    )
...
def standardize_channel_name(channel):
    ...
    # 第二轮清理，移除特定的词汇和结尾标识符
    re_patterns = (
        r"超高清|超清|高清|高清$|蓝光|频道|频道$|标清|FHD|HD|HD$|HEVC|HDR|-|\s+"
    )
    tid = re.sub(re_patterns, "", tid)
    tid = tid.strip()
