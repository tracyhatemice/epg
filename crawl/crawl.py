    ...
    for d in range(max_crawl_days):
        ...
        while True:
            if channel_queryset_no >= channel_no:  # 遍历完成所有频道后
                ...
                channel = channels[
                    0
                ]  # 使用了以后，不能再使用此标记了 如channel[0]使用了，channel[0]就变成别的了
            ...
            if ret["ban"] == 1:  # 被BAN掉处理方式
                ...
                ret["ban"] == 0 and channel in ban_channels
            ):  # 已经获取到的就从BAN列表中删除掉
            ...
            if len(ret["epgs"]) > 0:
                ...
                msg1 = "%s-%s" % (
                    msg1,
                    ret["source"],
                )  # 因为可能会更换来源，所以此处选择的是，返回的EPG中的来源
                success_num += 1
                msgx = ""
                if (
                    recrawl and channel.recrawl and d < recrawl_days
                ):  # 重新获取的频道需要删除旧数据
                    del_ret = Epg.del_channel_epgs(
                        Epg, channel.id, dt, ret["last_program_date"]
                    )
        ...
        if (
            cname
        ):  # 如果测试则只获取需要测试的那天数据,并不再做下面的生成xml及html等工作
            return
        msgn1 = (
            "，获取失败的为：%s" % (",".join(failed_channels))
            if len(failed_channels) > 0
            else ""
        )
        msg_failed = (
            ",%s失败" % (channel_no - success_num)
            if channel_no - success_num > 0
            else ""
        )
        msgn = "第 %s 天，共有%s/%s成功%s%s" % (
            d + 1,
            success_num,
            channel_no,
            msg_failed,
...
def get_epg(channel, dt, func_arg=0):
    ...
    while n <= crawl_info["retry_crawl_times"]:  # 重试
        ...
        if (
            "ban" in ret and ret["ban"] == 1
        ):  # 如果被BAN掉，直接进入下一条采集，不再重试等待，否则会浪费很多时间
            return ret
        ...
        if len(ret["epgs"]) > 0:  # and ret['success'] == 1: #有数据，则正常进入下一步骤
            ...
            log(
                "%s-%s-%s 第 %s 次重试失败！%s"
                % (channel.id, channel.name, channel.source, n, ret["msg"])
            )
            n += 1
    ...
    if n > crawl_info["retry_crawl_times"]:
        ...
        if crawl_info["change_source"]:  # 换源算法
            ...
            for source in channel_ids:
                ...
                ret = epg_func(
                    channel, channel_id, dt, func_arg=func_arg, source=source
                )
                ...
                if ret["success"] == 1:
                    ...
                    log(
                        "%s-%s 更换 %s 来源后，获取信息失败,%s"
                        % (channel.id, channel.name, source, ret["msg"]),
                        level=2,
                    )
            ...
            if not ret["success"]:  # 所有来源都无法获取到
                ...
                log(
                    "%s %s 来源获取成功"
                    % (",".join(sources) + " 来源获取失败，", ret["source"]),
                    level=1,
                )
    return ret
...
def gen_xml(sort):
    if cname:  # 测试时不生成
        return 0
    xmlhead = '<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE tv SYSTEM "https://github.com/kodi-pvr/pvr.iptvsimple"><tv generator-info-name="epg-xml" generator-info-url="https://epg.lost.to/">'
    xmlbottom = "</tv>"
    ...
    log(
        "crawl-gen_xml共有：%s 个频道,%s条信息需要生成"
        % (channels[1].count(), epgs.count())
    )
    f = open(xmldir, "w", encoding="utf-8")
    f.write(xmlhead)
    for channel in channels[0]:
        if (
            channel.sort in in_exclude_channel["xml"]
            and channel.name not in in_exclude_channel["include_xml"]
            and channel.tvg_name not in in_exclude_channel["include_xml"]
        ):
            continue
        c = '<channel id="%s"><display-name lang="zh">%s</display-name></channel>' % (
            channel.id,
            channel.tvg_name,
        )
        f.write(c)
    noepg_channel = (
        '<channel id="9999"><display-name lang="zh">noepg</display-name></channel>'
    )
    ...
    for epg in epgs:
        ...
        programinfo = (
            """<programme start="%s" stop="%s" channel="%s"><title lang="zh">%s</title><desc lang="zh">%s</desc></programme>"""
            % (start, end, id, title, desc)
        )
        f.write(programinfo)
    for x in range(10):
        noepg_program_day = noepg(
            "noepg",
            "9999",
            (datetime.datetime.now().date() + datetime.timedelta(days=x - 5)),
        )
        f.write(noepg_program_day)
    f.write(xmlbottom)
    f.close()
    log(
        "crawl-gen_xml 已经生成:%s 分类xml文件：%s"
        % (xmlinfo[sort]["sortname"], xmldir)
    )
...
def gen_test_m3u(channels, test_dir):
    with open(test_dir, "w", encoding="utf-8", errors="ignore") as f:
        ...
        for channel in channels:
            ...
            line = (
                '#EXTINF:-1 tvg-logo="%s" tvg-id="%s" tvg-name="%s" group-title="%s",%s\n'
