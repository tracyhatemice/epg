...
def get_epgs_cabletv(channel, channel_id, dt, func_arg):
    url = "http://www.cabletv.com.hk/ct/_cabletv_list_common.php?%s&date=%s" % (
        channel_id,
        dt.strftime("%Y%m%d"),
    )
    msg = ""  # 临时MSG，记录出错信息
    ...
    try:
        ...
        for (
            ...
            trs
        ):  # <tr><td width="50" valign="top" align="center"><div>05:30 </div></td><td width="15" valign="top"><div>&nbsp;</div></td><td valign="top"><div>有線新聞</div></td>  <td width="30"valign="top"><div>&nbsp;</div></td>              </tr>
            divs = tr.select("div")
            starttime_str = (
                divs[0].text.replace("\n", "").replace(" ", "")
            )  # 提取时间,5:55
