...
def get_desc(url_part):  # 获取节目介绍,总是出问题，暂不使用
    ...
    try:
        ...
        if soup.select("div.section"):
            ...
            desc = (
                soup.select("div.d_s_info")[0].text
                + soup.select("div.desc_col")[0].text
            )
...
def get_morning_lis(
    url, today
):  # 获取当天上午的lis，如果不是当天，另外处理,today参数为是否当天
    res = requests.get(url, headers=headers, timeout=5)
    res.encoding = "utf-8"
    soup = bs(res.text, "html.parser")
...
def get_epgs_tvmao(channel, channel_id, dt, func_arg):
    ...
    try:
        ...
        if isinstance(lss, str):
            soup = bs(
                lss, "html.parser"
            )  # 20201109以前为一段html,现在改为列表，然后再HTML
            lis1 = soup.select("li")
            for tr in lis1:
            ...
            for tr in lss:  # 列表形式的结果
                ...
                program_url = "https://www.tvmao.com" + href.replace(
                    "tvcolumn", "drama"
                )
                desc = get_desc(href)
                epg = {
...
# 有网友反馈使用此方式tvmao拉黑过多，写了新脚本，正在调试中，敬请期待
