...
def get_epgs_sdtv(channel, channel_id, dt, func_arg):
    ...
    try:
        url = (
            "http://module.iqilu.com/media/apis/main/getprograms?jsonpcallback=jQuery37004547755401387288_%s&channelID=%s&date=%s&_=%s"
            % (t, channel_id, dt, t)
        )
        ...
        for content in contents:
            starttime = datetime.datetime.fromtimestamp(
                int(content["begintime"])
            ) + datetime.timedelta(days=time_delta_days)
