...
def get_epgs_tvb(channel, channel_id, dt, func_arg):  # channel_id,dt
    ...
    url = "https://programme.tvb.com/api/schedule?input_date=%s&network_code=%s" % (
        dt_str,
        channel_id,
    )
    try:
...
def get_epgs_tvb_old(
    channel, channel_id, dt, func_arg
):  # 只需要channel_id即可 ,同时获取15天，包含前面时间的节目
    epgs = []
    msg = ""
    success = 1
