...
"""
2024/03/30 说明：
不懂就看:https://discuz.mxdyeah.top/mxdyeah_discuz_thread-70-1-1.html
这个地方也需要填写token,示例:
chuanliu_Authorization='Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI5ODQwODlhNjc1OGU0ZjJlOTViMjk4NWM4YjA1MDNmYiIsImNvbXBhbnkiOiJxaXlpIiwibmFtZSI6InRlcm1pbmFsIn0.1gDPpBcHJIE8dLiq7UekUlPWMtJOYymI8zoIYlsVgc4'
"""
headers = {
...
def argvs_get(
    argv,
):  # 命令行运行时可使用的参数 recrawl(重新获取)  channel(单独获取某一频道节目单） dt(单独某频道时，获取日期） save_to_db单独获取时，是否需要存至数据库
    ...
    if (
        "-n" in argv
        and len(argv) >= argv.index("-n") + 2
        and "-" not in argv[argv.index("-n") + 1]
    ):
        cname = argv[argv.index("-n") + 1]
    if (
        "-d" in argv
        and len(argv) >= argv.index("-d") + 2
        and "-" not in argv[argv.index("-d") + 1]
    ):
    ...
    if (
        "-s" in argv
        and len(argv) >= argv.index("-s") + 2
        and "-" not in argv[argv.index("-s") + 1]
    ):
...
def noepgjson(name, id, need_date):
    ...
    epgjsons = []
    for x in range(24):
        start = datetime.datetime.combine(need_date, datetime.time(x, 0, 0)).strftime(
            "%H:%M"
        )
        end = datetime.datetime.combine(need_date, datetime.time(x, 59, 59)).strftime(
            "%H:%M"
        )
        epgjsons.append(
            {
...
def noepg(name, id, need_date):
    ...
    for x in range(24):
        ...
        hour_epg = (
            """<programme start="%s" stop="%s" channel="9999"><title lang="zh">精彩节目-暂未提供节目预告信息</title><desc lang="zh"></desc></programme>"""
